...
def random_gemma3_rmsnorm(
    hidden_size: int,
    eps=1e-5,
    dtype=torch.float16,
    device="cpu",
) -> Gemma3RMSNorm:
    norm = Gemma3RMSNorm(dim=hidden_size, eps=eps)
    norm = norm.to(dtype=dtype, device=device)
...
def copy_gemma3_rmsnorm(norm: Gemma3RMSNorm) -> Gemma3RMSNorm:
    hidden_size = norm.weight.shape[0]
    new_norm = Gemma3RMSNorm(hidden_size, eps=norm.eps)
...
def _test_basic_gemma3_rmsnorm(dtype: torch.dtype, device: str):
    hidden_size = 256
    norm = random_gemma3_rmsnorm(hidden_size=hidden_size, dtype=dtype, device=device)

    # replace destroys the passed linear module so we need to copy it
    norm_copy = copy_gemma3_rmsnorm(norm)
...
def test_basic_gemma3_rmsnorm_fp32_cpu():
    _test_basic_gemma3_rmsnorm(dtype=torch.float32, device="cpu")


def test_basic_gemma3_rmsnorm_fp32_gpu():
    _test_basic_gemma3_rmsnorm(dtype=torch.float32, device="cuda")


def test_basic_gemma3_rmsnorm_fp16_gpu():
    _test_basic_gemma3_rmsnorm(dtype=torch.float16, device="cuda")


def test_basic_gemma3_rmsnorm_bf16_gpu():
    _test_basic_gemma3_rmsnorm(dtype=torch.bfloat16, device="cuda")
