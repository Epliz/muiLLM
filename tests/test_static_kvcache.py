...
def test_static_kv_cache_batch_size1_bf16_gpu():
...
def test_static_kv_cache_batch_size4_bf16_gpu():
    dtype = torch.bfloat16
    device = "cuda"
    batch_size = 4
    _test_static_kv_cache(batch_size=batch_size, dtype=dtype, device=device)
...
def _test_static_kv_cache_rope_update(batch_size: int, dtype: torch.dtype, device: str):
    max_cache_len = 128
    hidden_size = 256
    model_config = llama3_model_config(hidden_size=hidden_size, intermediate_size=1024)
...
def test_static_kv_cache_rope_update_batch_size1_fp32_cpu():
    dtype = torch.float32
    device = "cpu"
    batch_size = 1
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)


def test_static_kv_cache_rope_update_batch_size4_fp32_cpu():
    dtype = torch.float32
    device = "cpu"
    batch_size = 4
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)
...
def test_static_kv_cache_rope_update_batch_size1_fp32_gpu():
    dtype = torch.float32
    device = "cuda"
    batch_size = 1
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)
...
def test_static_kv_cache_rope_update_batch_size4_fp32_gpu():
    dtype = torch.float32
    device = "cuda"
    batch_size = 4
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)
...
def test_static_kv_cache_rope_update_batch_size1_fp16_gpu():
    dtype = torch.float16
    device = "cuda"
    batch_size = 1
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)
...
def test_static_kv_cache_rope_update_batch_size4_fp16_gpu():
    dtype = torch.float16
    device = "cuda"
    batch_size = 4
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)
...
def test_static_kv_cache_rope_update_batch_size1_bf16_gpu():
    dtype = torch.bfloat16
    device = "cuda"
    batch_size = 1
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)
...
def test_static_kv_cache_rope_update_batch_size4_bf16_gpu():
    dtype = torch.bfloat16
    device = "cuda"
    batch_size = 4
    _test_static_kv_cache_rope_update(batch_size=batch_size, dtype=dtype, device=device)
...
def _test_static_kv_cache_complex_rope_update(
...
def test_static_kv_cache_complex_rope_update_batch_size1_fp32_cpu():
    dtype = torch.float32
    device = "cpu"
    batch_size = 1
    _test_static_kv_cache_complex_rope_update(
        batch_size=batch_size, dtype=dtype, device=device
    )


def test_static_kv_cache_complex_rope_update_batch_size4_fp32_cpu():
    dtype = torch.float32
    device = "cpu"
    batch_size = 4
    _test_static_kv_cache_complex_rope_update(
...
def test_static_kv_cache_complex_rope_update_batch_size1_fp32_gpu():
    dtype = torch.float32
    device = "cuda"
    batch_size = 1
    _test_static_kv_cache_complex_rope_update(
...
def test_static_kv_cache_complex_rope_update_batch_size4_fp32_gpu():
    dtype = torch.float32
    device = "cuda"
    batch_size = 4
    _test_static_kv_cache_complex_rope_update(
...
def test_static_kv_cache_complex_rope_update_batch_size1_fp16_gpu():
    dtype = torch.float16
    device = "cuda"
    batch_size = 1
    _test_static_kv_cache_complex_rope_update(
...
def test_static_kv_cache_complex_rope_update_batch_size4_fp16_gpu():
    dtype = torch.float16
    device = "cuda"
    batch_size = 4
    _test_static_kv_cache_complex_rope_update(
...
def test_static_kv_cache_complex_rope_update_batch_size1_bf16_gpu():
    dtype = torch.bfloat16
    device = "cuda"
    batch_size = 1
    _test_static_kv_cache_complex_rope_update(
...
def test_static_kv_cache_complex_rope_update_batch_size4_bf16_gpu():
    dtype = torch.bfloat16
    device = "cuda"
    batch_size = 4
    _test_static_kv_cache_complex_rope_update(
