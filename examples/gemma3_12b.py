# Example showing how to use muiLLM's fp16 support on the Google Gemma 3 12b model
...
# this example requires the Google Gemma 3 12b Instruct model
# Provided that you have a HF token to access the Gemma models, you can download it with
# huggingface-cli download --token <your_token> google/gemma-3-12b-it --local-dir google-gemma-3-12b-it

# either set this environment variable before running the example, or adapt the path
model_id = os.getenv("GEMMA3_12B_PATH", "/storage/models/google-gemma-3-12b-it/")
...
text, time = profile_func(
    lambda: time_func(lambda: generate(model, tokenizer, prompt, 50)),
...
# Save a pytorch trace (visualizable for example with https://ui.perfetto.dev)
text, time = profile_func(
    lambda: time_func(lambda: generate(model, tokenizer, prompt, num_output_tokens)),
