...
def generate_input(
    rank: int, world_size: int, m: int, n: int, k: int, has_bias: bool, seed: int
) -> input_t:
    """
    ...
    input = (
        torch.rand((m, local_k), dtype=torch.bfloat16, device=device, generator=gen) * 2
        - 1
    ) * 0.01
    weight = (
        torch.rand((n, local_k), dtype=torch.bfloat16, device=device, generator=gen) * 2
        - 1
    ) * 0.01
    ...
    if has_bias:
        ...
        bias = (
            torch.rand((n,), dtype=torch.bfloat16, device=device, generator=gen) * 2 - 1
        ) * 0.01
