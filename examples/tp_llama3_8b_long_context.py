...
# and its performance on long context
...
def run(rank, size):
    ...
    # Initial prompt that we will use to generate a long context
    prompt = "Let's write a blogppost about how to write an optimized GEMV kernel for AMD GPUs with HIP"
    ...
    # Initial generation to get a longer prompt for next generation
    num_initial_output_tokens = 1024
    initial_output, time = time_func(lambda: generate(model, tokenizer, prompt, num_initial_output_tokens))

    long_prompt = prompt + initial_output # concatenate the initial prompt with the generated text

    tokenized_prompts = tokenizer(long_prompt, return_tensors="pt", padding="longest")
    if rank == 0:
    ...
    num_output_tokens = 16 # generate only a few tokens

    # Have a look at the speed
    text, time = time_func(lambda: generate(model, tokenizer, long_prompt, 10))
    text, time = time_func(lambda: generate(model, tokenizer, long_prompt, num_output_tokens))

    # check how many tokens were actually generated
    full_text = long_prompt + text
    tokenized_result= tokenizer(full_text, return_tensors="pt", padding="longest")
    num_total_tokens = tokenized_result["input_ids"].shape[1] * batch_size

    if rank == 0:
        print("[Optimized] Completion: ", full_text)
    ...
    text, time = profile_func(lambda: time_func(lambda: generate(model, tokenizer, long_prompt, num_output_tokens)), trace_path=f"trace_llama_muillm_tp{size}_long_ctx_rank{rank}.json")
