...
def run(rank, size):
    ...
    text, time = profile_func(lambda: time_func(lambda: generate(model, tokenizer, prompt, num_output_tokens)), trace_path=f"trace_llama_muillm_tp{size}_unbatched_rank{rank}.json")
