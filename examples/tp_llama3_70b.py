...
def run(rank, size):
    ...
    text, time = profile_func(lambda: time_func(lambda: generate(model, tokenizer, prompt, 50)), trace_path=f"trace_llama70b_muillm_tp{size}_unbatched_rank{rank}.json")
