...
def custom_kernel(data: input_t) -> output_t:
    ...
    with profiler.record_function("all2all_dispatch"):
        expert_num, expert_x, expert_meta = ata.dispatch(rank_data.x, rank_data.indices)

    if ata.comms is not None:
        # we only apply the expert computation on the received tokens
        expert_y = ata.comms.compute(expert_num, expert_x)
    else:
    ...
    with profiler.record_function("all2all_combine"):
        y = ata.combine(
            y,
            rank_data.weights,
            expert_meta,
