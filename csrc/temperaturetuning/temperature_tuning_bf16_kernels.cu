...
__global__ void muillm_apply_temperature_tuning_bf16_kernel(
  const __hip_bfloat16* __restrict__ q_in, // [B, num_heads, T, embed_dim]
  const int64_t* cache_position,
  __hip_bfloat16* __restrict__ q_out, // [B, num_heads, T, embed_dim]
  float attn_scale,
  float floor_scale,
  unsigned B,
  unsigned num_heads,
  ...
  for (int i = threadIdx.x; i < embed_dim; i += THREADS_PER_BLOCK) {
    ...
    q_out[i] = __float2bfloat16(__bfloat162float(q_in[i]) * attention_scale);
  }
}
...
void muillm_apply_temperature_tuning_bf16(
  ...
  muillm_apply_temperature_tuning_bf16_kernel<<<num_blocks, threads_per_block, 0, stream>>>(
    (const __hip_bfloat16*)q_in,
    (const int64_t*)cache_position,
    (__hip_bfloat16*)y,
    attn_scale,
