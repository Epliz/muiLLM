...
__global__ void muillm_reduce_sum_bf16_kernel(
    const __hip_bfloat16* __restrict__ X, // size BxMxN
    __hip_bfloat16* __restrict__ Y, // size BxMx1 (if we reduce columns) or Bx1xN (if we reduce rows)
  ...
  if (reduce_last_dim) {
    ...
    for (int col_idx = threadIdx.x; col_idx < N; col_idx += THREADS_PER_BLOCK) {
      ...
      sum += __bfloat162float(X[col_idx]);
    }
    ...
    if (threadIdx.x == 0) {
      ...
      Y[0] = __float2bfloat16(shared_sum);
    }
  } else {
    ...
    if (col_idx < N) {
      for (int row_idx = 0; row_idx < M; ++row_idx) {
        // TODO: vectorize by processing several columns with one thread
        sum += __bfloat162float(X[row_idx * N + col_idx]);
      }

      // write the result to the output tensor
      Y[col_idx] = __float2bfloat16(sum);
    }
...
void muillm_reduce_sum_bf16(
  ...
  if (reduce_last_dim) {
    ...
    // launch the kernel
    muillm_reduce_sum_bf16_kernel<<<num_blocks, threads_per_blocks, 0, stream>>>(
        x,
        y,
        M,
        N,
