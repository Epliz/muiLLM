...
void __global__ causally_compute_transformer_softmax_scores_bf16_kernel(
  ...
  for (; kv_tok_idx < S; kv_tok_idx += warps_per_block) {
    ...
    for (; d + 1 < embed_dim; d += 2 * warpSize) {
      __hip_bfloat162 qs = *(const __hip_bfloat162*)addr(q_in, d);
      __hip_bfloat162 ks = *(const __hip_bfloat162*)addr(k_in, d);
      attention_score0 += __bfloat162float(qs.x) * __bfloat162float(ks.x);
      attention_score1 += __bfloat162float(qs.y) * __bfloat162float(ks.y);
    }
    if (d < embed_dim) {
      // remainder
      attention_score0 += __bfloat162float(q_in[d]) * __bfloat162float(k_in[d]);
    }
  ...
  for (unsigned t = threadIdx.x; t < S; t += THREADS_PER_BLOCK) {
    ...
    *addr(scores_out, t) = __float2bfloat16(logit);
  }
}
...
void causally_compute_transformer_softmax_scores_bf16(
  ...
  causally_compute_transformer_softmax_scores_bf16_kernel<<<num_blocks, threads_per_blocks, 0, stream>>>(
    (const __hip_bfloat16*)q_in,
    (const __hip_bfloat16*)k_in,
    (float*)temp_scores_f32,
    (__hip_bfloat16*)scores_out,
    // tensor dimension sizes
    T,
    S,
    num_q_heads,
...
// expected block dimensions: [x=T, y=num_q_heads, z=B]
void __global__ causally_compute_transformer_softmax_scores_masked_bf16_kernel(
  const __hip_bfloat16* __restrict__ q_in, // shape [B, num_q_heads, T, embed_dim]
  const __hip_bfloat16* __restrict__ k_in, // shape [B, num_k_heads, S, embed_dim]
  ...
  for (; kv_tok_idx < S; kv_tok_idx += warps_per_block) {
    ...
    // as that's faster
    float mask_val = __bfloat162float(*addr(m_in, kv_tok_idx));
...
void causally_compute_transformer_softmax_scores_masked_bf16(
  ...
  causally_compute_transformer_softmax_scores_masked_bf16_kernel<<<num_blocks, threads_per_blocks, 0, stream>>>(
    (const __hip_bfloat16*)q_in,
    (const __hip_bfloat16*)k_in,
...
void __global__ causally_apply_transformer_softmax_scores_bf16_kernel(
  ...
  if (blockColStartIdx + colIdx < embed_dim) {
    ...
    for (unsigned r = rowIdx; r < S; r+= ROWS_PER_BLOCK) {
      float v = __bfloat162float(v_in[colIdx]);
      float a = __bfloat162float(attention_weights_in[r]);
...
void causally_apply_transformer_softmax_scores_bf16(
  ...
  causally_apply_transformer_softmax_scores_bf16_kernel<<<num_blocks, threads_per_blocks, 0, stream>>>(
    (const __hip_bfloat16*)attention_weights_in,
