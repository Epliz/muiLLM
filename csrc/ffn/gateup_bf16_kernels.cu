...
__global__ void muillm_gateupmlp_gemv_norm_inputs_split_bf16_kernel(
    ...
    const __hip_bfloat16* __restrict__ UW, // weight matrix - size N x K
    const __hip_bfloat16* __restrict__ X, // input = size K
    __hip_bfloat16* __restrict__ GY, // output - size N
    __hip_bfloat16* __restrict__ UY, // output - size N
    unsigned N,
    unsigned K,
    float epsilon,
    float weights_offset,
  ...
  const __hip_bfloat16* __restrict__ W = blockIdx.y == 0 ? GW : UW; // weight matrix - size N x K
  __hip_bfloat16* __restrict__ Y = blockIdx.y == 0 ? GY : UY; // output - size N
...
__global__ void muillm_gateupmlp_gemv_split_bf16_kernel(
  ...
  const __hip_bfloat16* __restrict__ W = blockIdx.y == 0 ? GW : UW;
  __hip_bfloat16* __restrict__ Y = blockIdx.y == 0 ? GY : UY;
