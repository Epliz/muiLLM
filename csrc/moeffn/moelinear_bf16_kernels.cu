...
__global__ void muillm_moegemv_bf16_kernel(
    const __hip_bfloat16* __restrict__ W, // weight matrix - size (num_shared_experts + num_dynamic_experts) x N x K
    const __hip_bfloat16* __restrict__ X, // input = size (num_shared_experts + num_routed_experts) x K
...
template<int THREADS_PER_BLOCK>
__global__ void muillm_moegemv_norm_inputs_bf16_kernel(
  ...
  */
  ...
  if (normalize) {
    ...
    if (threads_per_blocks == 64) {
      muillm_moegemv_norm_inputs_bf16_kernel<64><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const __hip_bfloat16*)norm_weights,
      ...
      muillm_moegemv_norm_inputs_bf16_kernel<128><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const __hip_bfloat16*)norm_weights,
        (const __hip_bfloat16*)weights,
        (const __hip_bfloat16*)x,
        (const int64_t*)router_indices,
      ...
      muillm_moegemv_norm_inputs_bf16_kernel<256><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const __hip_bfloat16*)norm_weights,
        (const __hip_bfloat16*)weights,
        (const __hip_bfloat16*)x,
        (const int64_t*)router_indices,
    ...
    if (threads_per_blocks == 64) {
      muillm_moegemv_bf16_kernel<64><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const __hip_bfloat16*)weights,
        (const __hip_bfloat16*)x,
        (const int64_t*)router_indices,
        activ,
      ...
      muillm_moegemv_bf16_kernel<128><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const __hip_bfloat16*)weights,
        (const __hip_bfloat16*)x,
        (const int64_t*)router_indices,
        activ,
      ...
      muillm_moegemv_bf16_kernel<256><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const __hip_bfloat16*)weights,
        (const __hip_bfloat16*)x,
        (const int64_t*)router_indices,
        activ,
