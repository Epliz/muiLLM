...
template<int THREADS_PER_BLOCK>
__global__ void muillm_moegemv_norm_inputs_fp16_kernel(
  ...
  */
  ...
  if (normalize) {
    ...
    if (threads_per_blocks == 64) {
      muillm_moegemv_norm_inputs_fp16_kernel<64><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const half*)norm_weights,
        (const half*)weights,
        (const half*)x,
        (const int64_t*)router_indices,
      ...
      muillm_moegemv_norm_inputs_fp16_kernel<128><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const half*)norm_weights,
        (const half*)weights,
        (const half*)x,
        (const int64_t*)router_indices,
      ...
      muillm_moegemv_norm_inputs_fp16_kernel<256><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const half*)norm_weights,
        (const half*)weights,
        (const half*)x,
        (const int64_t*)router_indices,
    ...
    if (threads_per_blocks == 64) {
      muillm_moegemv_fp16_kernel<64><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const half*)weights,
        (const half*)x,
        (const int64_t*)router_indices,
        activ,
      ...
      muillm_moegemv_fp16_kernel<128><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const half*)weights,
        (const half*)x,
        (const int64_t*)router_indices,
        activ,
      ...
      muillm_moegemv_fp16_kernel<256><<<num_blocks, threads_per_blocks, 0, stream>>>(
        (const half*)weights,
        (const half*)x,
        (const int64_t*)router_indices,
        activ,
