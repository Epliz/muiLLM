...
class MuiGemma3DecoderLayer(MuiModule):
    def __init__(
        ...
        input_layernorm: MuiRMSNorm,
        post_attention_layernorm: MuiRMSNorm,
        pre_feedforward_layernorm: MuiRMSNorm,
    ...
    def replace(
        ...
        input_layernorm = MuiRMSNorm.replace(
            replacement_context,
            prev_module.input_layernorm,
        )
        post_attention_layernorm = MuiRMSNorm.replace(
            replacement_context,
            prev_module.post_attention_layernorm,
        )
        pre_feedforward_layernorm = MuiRMSNorm.replace(
            replacement_context,
            prev_module.pre_feedforward_layernorm,
        )
