...
class MuiParallelExperts(MuiModule):
    ...
    def parallel_forward(
        ...
        next_states = torch.bmm(
            (self.activation_function(g) * u),  # shape (num_experts, -1, tp_expert_dim)
            down_proj_weights,
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if self.tensor_parallelism > 1:
            return self.parallel_forward([hidden_states])[0]
...
class MuiParallelGateUpDownMLPMoe(MuiModule):
    ...
    def parallel_forward(
        ...
        routed_out = self.experts.parallel_forward([routed_in])[0]
        out = self.shared_expert.parallel_forward([hidden_states])[0]
