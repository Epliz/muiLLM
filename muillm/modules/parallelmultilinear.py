...
class MuiParallelMultiLinear(MuiModule):
    ...
    @staticmethod
    def replace(prev_modules: Union[MuiMultiLinear, List[nn.Linear]], engine_config: MuiEngineConfig, prev_layernorm_module: Union[LlamaRMSNorm, MistralRMSNorm] = None, sharding_dim: int = 0) -> "MuiParallelMultiLinear":
    ...
    @staticmethod
    def _replace_multilinear(prev_module: MuiMultiLinear, engine_config: MuiEngineConfig, prev_layernorm_module: Union[LlamaRMSNorm, MistralRMSNorm] = None, sharding_dim: int = 0) -> "MuiParallelMultiLinear":
    ...
    @staticmethod
    def _replace_linears(prev_modules: List[nn.Linear], engine_config: MuiEngineConfig, prev_layernorm_module: Union[LlamaRMSNorm, MistralRMSNorm] = None, sharding_dim: int = 0) -> "MuiParallelMultiLinear":
    ...
    def copy_modules(self, prev_modules: List[nn.Linear], norm_weights: torch.Tensor = None, variance_epsilon: float = 0.0):
        ...
        weigths_per_device = [torch.cat([all_weights[i] for i in range(num_linears)], dim=0)]
        ...
        if has_bias:
            ...
            biases_per_device = [self._cat_dim0([all_biases[i] for i in range(num_linears)])]
    ...
    def parallel_forward(self, input: Union[Tensor, List[Tensor]], collect_outputs: bool = True) -> List[Tuple[Tensor, ...]]:
        ...
        if self.sharding_dim == 1:
            all_outputs = self.linear.parallel_forward(input, collect_outputs=collect_outputs)[0]
        ...
        if collect_outputs:
            ...
            for output_idx in range(num_slices):
                all_split_outputs[output_idx] = self.__collect_output(all_split_outputs[output_idx])
