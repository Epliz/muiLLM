...
class Communicator(ABC):
    def __init__(
        ...
        self.world_size = (
            world_size if world_size is not None else int(os.environ["WORLD_SIZE"])
        )
        self.local_size = (
            local_size if local_size is not None else int(os.environ["LOCAL_SIZE"])
        )
        self.rank = rank if rank is not None else int(os.environ["RANK"])
        self.local_rank = (
            local_rank if local_rank is not None else int(os.environ["LOCAL_RANK"])
        )
...
class MuiCommunicator(TorchCommunicator):
    def __init__(
        ...
        if self.is_multi_node():
            raise ValueError(
                f"Multi-node setups are not supported with MuiCommunicator, use TorchCommunicator instead. Specified WORLD_SIZE {self.world_size} and LOCAL_SIZE {self.local_size}"
            )

        process_group = dist.group.WORLD
        self.comms = muillm_ext.muillm_comm_init(
            cpp_engine,
            process_group,
            self.world_size,
            self.local_size,
