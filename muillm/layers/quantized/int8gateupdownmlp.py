...
class MuiInt8GateUpDownMLP(MuiModule):
    def __init__(self, engine_config: MuiEngineConfig, hidden_size: int, intermediate_size: int, activation_function: nn.Module, variance_epsilon:float = 0.0, normalize:bool = False, device=None, dtype=None) -> None:
        ...
        gate_proj = MuiInt8Linear(engine_config=engine_config, in_features=self.hidden_size, out_features=self.intermediate_size, bias=False, device=device, dtype=dtype)
        up_proj = MuiInt8Linear(engine_config=engine_config, in_features=self.hidden_size, out_features=self.intermediate_size, bias=False, device=device, dtype=dtype)
        ...
        self.down_proj = MuiInt8Linear(engine_config=engine_config, in_features=self.intermediate_size, out_features=self.hidden_size, bias=False, device=device, dtype=dtype)
    ...
    def replace(prev_module: MuiGateUpDownMLP, engine_config: MuiEngineConfig) -> "MuiInt8GateUpDownMLP":
        ...
        new_module = MuiInt8GateUpDownMLP(engine_config=engine_config, hidden_size=hidden_size, intermediate_size=intermediate_size, activation_function=activation_function, variance_epsilon=variance_epsilon, normalize=normalize, dtype=dtype, device=device)
    ...
    def copy_module(self, prev_module: MuiGateUpDownMLP):
        ...
        gate_proj = MuiInt8Linear(engine_config=self.engine_config, in_features=self.hidden_size, out_features=self.intermediate_size, bias=False, device=self.device, dtype=self.dtype)
        up_proj = MuiInt8Linear(engine_config=self.engine_config, in_features=self.hidden_size, out_features=self.intermediate_size, bias=False, device=self.device, dtype=self.dtype)
